...
# =============== CONFIG ===============
OUTPUT_FOLDER = "guitar_di_samples_round_robin"
# ======================================
...
def get_unique_filename(base_name):
    """
    Given a base filename like "A2_057_downstroke.wav",
    returns the next available version:
        A2_057_downstroke_00.wav
        A2_057_downstroke_01.wav
        etc.
    """
    if not os.path.exists(os.path.join(OUTPUT_FOLDER, base_name)):
        return base_name  # First one, no suffix needed? Wait — you want _00 even for the first!

    # You specifically asked for _00.wav on the first occurrence too
    counter = 0
    while True:
        numbered_name = base_name.replace(".wav", f"_{counter:02d}.wav")
        full_path = os.path.join(OUTPUT_FOLDER, numbered_name)
...
def process_file(filepath):
    ...
    for i in range(len(onsets)-1):
        ...
        # Base filename without counter
        base_filename = f"{note_display}_{midi_str}_{articulation}.wav"
        final_filename = get_unique_filename(base_filename)

        out_path = os.path.join(OUTPUT_FOLDER, final_filename)
        wavfile.write(out_path, fs, np.int16(segment * 32767))
        print(f"   → {final_filename}  ({freq:.1f} Hz)")
